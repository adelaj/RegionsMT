...
foreach ($i in $lib)
{
    ...
    foreach ($j in $ARCH.Split(" ")) 
    {
        ...
        cd $i-$j
        ${lib-flags} = (Get-Variable -Name ("cmake-flags" + $lib)).Value.Trim()
        cmake $(if (${lib-flags}) { ${lib-flags} + " " })-D MSVC -D CMAKE_C_FLAGS_INIT="/GL" -D CMAKE_STATIC_LINKER_FLAGS_INIT="/LTCG" -D CMAKE_GENERATOR_PLATFORM="$j" $(join-path .. $i)
