...
// Warging! The order of the following constants in implicitly used in the next function
enum fmt_type {
    FMT_INT, // 'i' (int)
    FMT_INT8, // 'ib' (int8_t)
...
bool decode_fmt(struct fmt_context *context, char *fmt, size_t *p_len)
{
    const char flags[] = { '?', '<', '>' };
    struct fmt_res res = { 0 };
    size_t len = 0;
    for (size_t i = 0, j = 0; i <= j; i++) switch (i)
    {
    case 0:
    case 1:
    case 2:
        if (fmt[len] != flags[j++]) break;
        res.flags |= 1 << i;
        len++;
        break;
    case 4:
    ...
    case 5:
        ...
        case FMT_UTF:
            ...
            {
                const char *tmp;
                if (str_to_uint32(fmt + len, &tmp, &res.arg.utf8_val) && res.arg.utf8_val < UTF8_BOUND);
