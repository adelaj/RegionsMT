...
struct main_args main_args_default()
{
    ...
    bs(res.bits, MAIN_ARGS_BIT_POS_THREAD_CNT);
    return res;
}
...
struct main_args main_args_override(struct main_args args_hi, struct main_args args_lo)
{
    ...
    if (bt(args_hi.bits, MAIN_ARGS_BIT_POS_THREAD_CNT)) res.thread_cnt = args_hi.thread_cnt;
    else
    {
    ...
    if (log_init(&fallback, NULL, 0, LOG_WRITE_LOCK, &ttl_style, &style, NULL))
    {
        ...
        if (argv_parse(argv_par_selector, &argv_par_sch, &main_args, argv, argc, &pos_arr, &pos_cnt, &fallback))
        {
            ...
            if (log_init(&log, main_args.log_path, BLOCK_WRITE, LOG_WRITE_LOCK, &ttl_style, &style, &fallback))
            {
                ...
                /*
                if (bt(main_args.bits, MAIN_ARGS_BIT_POS_FANCY))
                {
                    if (bt(main_args.bits, MAIN_ARGS_BIT_POS_FANCY_USER))
                ...
                */
                ...
                if (bt(main_args.bits, MAIN_ARGS_BIT_POS_HELP))
                {
                    // Help mode
                    log_message_fmt(&log, CODE_METRIC, MESSAGE_INFO, "Help mode triggered!\n");
                }
                else if (bt(main_args.bits, MAIN_ARGS_BIT_POS_TEST))
                {
                    succ = test_main(main_args.thread_cnt, &log);
                }
                else if (bt(main_args.bits, MAIN_ARGS_BIT_POS_CAT))
                {
                    if (pos_cnt >= 7)
                    {
                ...
                else if (bt(main_args.bits, MAIN_ARGS_BIT_POS_LDE))
                {
                    if (pos_cnt >= 2) lde_run(pos_arr[0], pos_arr[1], &fallback);
                }
                else if (bt(main_args.bits, MAIN_ARGS_BIT_POS_CHISQ))
                {
                    if (pos_cnt >= 4)
                    {
                ...
                else if (bt(main_args.bits, MAIN_ARGS_BIT_POS_LM))
                {
                    if (pos_cnt >= 5)
                    {
